	...
	<div id="board_divide">
		...
		<div id="free_board" class="tabcontent">
			...
			<table id="write_btn" width="100%" cellspacing="1">
				<tr height="40">
					<td align="right">
						<button onclick="window.location='freeBoardWriteForm.do'">글쓰기</button>&nbsp;
						<button onclick="window.location='freeBoardList.do'">전체목록</button>
					</td>
				</tr>
			</table>
				...
				<c:if test="${count !=0 }">
					<c:forEach var="vo" items="${boardList }">
						<tr>
							...
							<td align="left"><a align="center"
								href="freeBoardContent.do?num=${vo.num}&pageNum=${bp.cur_Page}">${vo.subject}
			...
			<c:forEach var="counter" begin="${bp.getPage_Start() }"
				...
				<c:if test="${search_text ne ''}">
					<a
						href="javascript:window.location='freeBoardList.do?pageNum=${counter }&search_type=${search_type}&search_text=${search_text}'">
						[${counter }]</a>
				</c:if>
				<c:if test="${search_text eq ''}">
					<a
						href="javascript:window.location='freeBoardList.do?pageNum=${counter }'">
						[${counter }]</a>
				</c:if>
			</c:forEach>
			<c:if test="${bp.isNext() }">
