	...
	<div id="board_content">
      ...
      <tr>
          <td align="right" colspan="5">
 		    <input type="button" value="전체목록" onclick="window.location='freeBoardList.do'" />
            <input type="button" value="글쓰기" onclick="window.location='freeBoardWriteForm.do'"/>
      	  </td>
      </tr>
      <tr>
      ...
      <c:if test="${count !=0 }">
         <c:forEach var="vo" items="${boardList }">
            <tr>
               ...
               <a align="center" href="freeBoardContent.do?num=${vo.num}&pageNum=${bp.cur_Page}">${vo.subject} (${vo.commentcount}) </a>
      ...
      <br/>
      <c:if test="${bp.isPre() }">
      <a href="freeBoardList.do?pageNum=${bp.getPage_Start()-bp.p_size}">[이전]</a>
      </c:if>
      <c:forEach var="counter" begin="${bp.getPage_Start() }" end="${bp.getPage_End() }">
      <c:if test="${search_text ne ''}">
      <a href="javascript:window.location='freeBoardList.do?pageNum=${counter }&search_type=${search_type}&search_text=${search_text}'">
      [${counter }]</a>
      </c:if>
      <c:if test="${search_text eq ''}">
      <a href="javascript:window.location='freeBoardList.do?pageNum=${counter }'">
      [${counter }]</a>
      </c:if>
      </c:forEach>
      <c:if test="${bp.isNext() }">
      <a href="freeBoardList.do?pageNum=${bp.getPage_Start()+bp.p_size }">[다음]</a>
      </c:if>
      <p><p>
