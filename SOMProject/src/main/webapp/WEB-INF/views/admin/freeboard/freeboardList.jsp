	...
	<div id="board_divide">
		...
		<div id="free_board" class="admin_tabcontent">
			...
			<table id="write_btn" width="100%" cellspacing="1">
				<tr height="40">
					<td align="right">
						<button onclick="window.location='adminFreeBoardWriteForm.mdo'">글쓰기</button>&nbsp;
						<button onclick="window.location='adminFreeBoardList.mdo'">전체목록</button>
					</td>
				</tr>
			</table>
				...
				<c:if test="${count !=0 }">
					<c:forEach var="vo" items="${boardList }">
						<tr>
							...
							<td align="left"><a
								href="adminFreeBoardContent.mdo?num=${vo.num}&pageNum=${bp.cur_Page}">${vo.subject}
			...
			<div class="pagination">
				<c:if test="${bp.isPre() }">
					<a
						href="adminFreeBoardList.mdo?pageNum=${bp.getPage_Start()-bp.p_size}">[이전]</a>
				</c:if>
				<c:forEach var="counter" begin="${bp.getPage_Start() }"
					end="${bp.getPage_End() }">
					<c:if test="${search_text ne ''}">
						<a
							href="javascript:window.location='adminFreeBoardList.mdo?pageNum=${counter }&search_type=${search_type}&search_text=${search_text}'">
							[${counter }]</a>
					</c:if>
					<c:if test="${search_text eq ''}">
						<a
							href="javascript:window.location='adminFreeBoardList.mdo?pageNum=${counter }'">
							[${counter }]</a>
					</c:if>
				</c:forEach>
				<c:if test="${bp.isNext() }">
					<a
						href="adminFreeBoardList.mdo?pageNum=${bp.getPage_Start()+bp.p_size }">[다음]</a>
				</c:if>
				<p>
				<p>
			</div>
