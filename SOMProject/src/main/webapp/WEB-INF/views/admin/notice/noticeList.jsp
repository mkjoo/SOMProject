...
<div>
      ...
      <tr>
          <td align="right" colspan="5">
 		    <input type="button" value="전체목록" onclick="window.location='adminNoticeList.mdo'" />
            <input type="button" value="글쓰기" onclick="window.location='adminNoticeWriteForm.mdo'"/>
      	  </td>
      </tr>
      <tr>
      ...
      <br/>
      <c:if test="${bp.isPre() }">
      <a href="adminNoticeList.mdo?pageNum=${bp.getPage_Start()-bp.p_size}">[이전]</a>
      </c:if>
      <c:forEach var="counter" begin="${bp.getPage_Start() }" end="${bp.getPage_End() }">
      <c:if test="${search_text ne ''}">
      <a href="javascript:window.location='adminNoticeList.mdo?pageNum=${counter }&search_type=${search_type}&search_text=${search_text}'">
      [${counter }]</a>
      </c:if>
      <c:if test="${search_text eq ''}">
      <a href="javascript:window.location='adminNoticeList.mdo?pageNum=${counter }'">
      [${counter }]</a>
      </c:if>
      </c:forEach>
      <c:if test="${bp.isNext() }">
      <a href="adminNoticeList.mdo?pageNum=${bp.getPage_Start()+bp.p_size }">[다음]</a>
      </c:if>
      <p><p>
